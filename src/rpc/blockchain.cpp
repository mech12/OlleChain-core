...
UniValue getmempoolancestors(const JSONRPCRequest& request)
{
    ...
    if (it == mempool.mapTx.end()) {
        std::string log = std::string("Transaction not in mempool") + hash.ToString();
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, log.c_str());
    }
...
UniValue getmempooldescendants(const JSONRPCRequest& request)
{
    ...
    CTxMemPool::txiter it = mempool.mapTx.find(hash);
    if (it == mempool.mapTx.end()) {
        std::string log = std::string("2 Transaction not in mempool") + hash.ToString();
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, log.c_str());
    }
...
UniValue getmempoolentry(const JSONRPCRequest& request)
{
    ...
    CTxMemPool::txiter it = mempool.mapTx.find(hash);
    if (it == mempool.mapTx.end()) {
        std::string log = std::string("3 Transaction not in mempool") + hash.ToString();
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, log.c_str());
    }
